...
dxvalues_determined = []
densityvalues_imposed = []

seriesnames=os.listdir(datafolder)

for seriesindex,seriesname in enumerate(seriesnames):
	densityvalues = os.listdir(datafolder+seriesname+"/simulation/")

	for densityindex,densityname in enumerate(densityvalues):
		print(densityname)
		density = float(densityname.split("_")[1])

		if not os.path.exists(datafolder+seriesname+"/analysis/"+densityname):
			os.makedirs(datafolder+seriesname+"/analysis/"+densityname)
		...
		stressvaluenames = os.listdir(datafolder+seriesname+"/simulation/"+densityname+"/")
		stressvalues = []
		...
		for stressindex,stress in enumerate(stressvalues):
			...
			for repeat in range(nr_repeats):
				...
				logfilename=datafolder+seriesname+"/simulation/"+densityname+"/"+stressvaluename+"/rep_"+str(repeat)+"/log.csv"

				#Read in the simulation parameters from the input file used for the simulations
				par={}
				inputfile = open(datafolder+seriesname+"/simulation/"+densityname+"/"+stressvaluename+"/rep_"+str(repeat)+"/input.txt",'r')
	...
	np.savetxt(datafolder+seriesname+"/analysis/"+densityname+"/force_lifetimecurve.csv",curve_for_storage,delimiter = ",", header = "F_shear[pN],tau_average[s],tau_stdevs[s]")
	...
	if fitting:
		if len(lifetime_averages)>3:
			...
			densityvalues_imposed.append(density)
...
ax.set_xlim(left = -0.1, right = 200)
...
densityvalues_imposed = np.array(densityvalues_imposed)		
...
storagematrix = np.hstack((densityvalues_imposed[:,np.newaxis],dxvalues_determined[:,np.newaxis],tau0values_determined[:,np.newaxis]))
storagefile = "parameters_information.csv"

np.savetxt(storagefile,storagematrix,delimiter=',',header="bond density(imposed - #/um2),dx(determined - nm),tau0(determined - s)")
