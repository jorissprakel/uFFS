...
legends_exp=[ #legends of the experimental curves
'exp. 15bp',
'exp. 25bp',
'exp. 30bp'
]

legends_sim=[ #legends of the simulated curves
'sim. 15bp',
'sim. 25bp',
'sim. 30bp'
]
...
for seriesindex,seriesname in enumerate(seriesnames):
	...
	analysisfolder = datafolder+seriesname+"/analysis/"
	...
	for iterindex, iterator in enumerate(iteratorvalues):
		...
		ax.semilogy(shearforces_pN,lifetime_averages,'ks-',color=colors[seriesindex],linewidth=0,markersize=8,markeredgewidth=2,markerfacecolor='white',label = legends_sim[seriesindex])

		dxvalue = dxvalues_determined[seriesindex]
		tau0value = tau0values_determined[seriesindex]
...
ax.set_xlabel(r'$F_{shear}\ [\mathregular{pN}]$',fontsize=25)
ax.set_ylabel(r'$\tau \ [\mathregular{s}]$',fontsize = 25,rotation="vertical")

legend = ax.legend(frameon=False,labelspacing=0.3,loc="upper right",fontsize = 18,handlelength=0,markerscale=1)
plt.savefig("force_lifetimecurve_overlayed_log.png",dpi=300)
plt.show()
plt.close()
